...
class GameAction:
    ...
    def locate_hero(self):
        while True:
            ...
            if hero is not None:
                #如果卡在一个点 x+y波动值小于30
                cur_hx_hy_sum = int(self.param.hx)+int(self.param.hy)
                if abs(cur_hx_hy_sum-self.param.last_hx_hy_sum) < 30 :
                    self.param.not_move_cnt +=1
                #卡10次以上随机移动 并初始化卡次数
                if self.param.not_move_cnt >10:
                    print(f"当前卡住次数: {self.param.not_move_cnt},随机移动")
                    self.no_hero_handle()
                    # self.ctrl.move(random.randint(0, 360), 2)
    ...
    def pick_up_equipment(self):
        ...
        while True:
            ...
            monster = self.find_tag(result, ['monster', 'boss', 'card'])
            if len(monster) > 0:
                print('找到怪物，或者发现卡片，停止捡装备。。')
                return
            # if len(hero)>0
            #     hero = hero[0]
            #     self.param.hx, self.param.hy = get_detect_obj_bottom(hero)

            equipment = self.find_tag(result, 'item')
            if len(equipment) > 0:
                print('找到装备数量：', len(equipment))
                self.move_to_target(equipment, hero, self.param.hx, self.param.hy, screen)
    ...
    def move_to_target(self, target: list, hero, hx, hy, screen):
        ...
        cur_hx = self.param.hx
        cur_hy = self.param.hy
        #改为直接计算坐标 非常依赖英雄位置的更新 可以新开一线程一直更新英雄的位置
        min_distance_obj = min(target, key=lambda a: distance_xy(self.param.hx,self.param.hy, a.xywh[0][0].item(),a.xywh[0][1].item()))
    ...
    def no_hero_handle(self,result=None, t = 0.8):
        ...
        if self.atack_flag :
          self.ctrl.skill_near()
          self.ctrl.attack(1)
        self.ctrl.move(random.randint(0,360),1)

        # self.move_to_xy(sx, sy)


    def move_to_xy(self, x, y, out_time=0.8):
        """
        移动到指定位置,默认2秒超时
        :param x:
        :param y:
    ...
    def attack_master(self):
        ...
        while True:
            ...
            if len(monster) > 0:
                ...
                # 最近距离的怪物坐标
                nearest_monster = min(monster, key=lambda a: distance_xy(self.param.hx,self.param.hy,a.xywh[0][0].item(),a.xywh[0][1].item()))
                distance = distance_xy(self.param.hx,self.param.hy,nearest_monster.xywh[0][0].item(),nearest_monster.xywh[0][1].item())
                ...
                self.param.mov_start = False
                # 面向敌人
                angle = calc_angle(self.param.hx, self.param.hy, ax, ay)
                self.ctrl.move(angle, 0.4)
